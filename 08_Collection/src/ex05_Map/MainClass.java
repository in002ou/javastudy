...
public class MainClass {
	...
	public static void ex01() {
		...
		/* 	HashMap
			...
			5. Key는 중복이 불가능하고, Value는 중복이 가능.
		*/
		...
		// 수정
		// put(Key, Value)
		person.put("age", 40);	// 기존 Key와 동일한 Key 값을 저장하면 중복이 되지 않고 Value가 수정이 된다.
		
		// 삭제
		// remove(Key)
		int age = (int)person.remove("age");	// 삭제된 Value가 반환, age의 값이 오브젝트이기 때문에 캐스팅을 해줘야 한다.
	...
	public static void ex04() {  // 연습
		...
		for(int i = 0, length = books.size(); i < length; i ++) {
			// Map 순회(list에 저장된 요소가 Map이다.)
			Map<String, Object> book = books.get(i);
			System.out.println((i + 1) + "번 째 책의 정보");
