...
public class MainClass {
	...
	public static void ex01() {
		...
		try {
			...
			System.out.println("파라미터 : " + url.getQuery()); 	// 명식은 파라미터지만 자바에서 쿼리로 나온다 하지만 절대 쿼리가 파라미터가 아니다.
	...
	public static void ex02() {
		...
		try {
			url = new URL(apiURL);								// MalformedURLException 필요
			con = (HttpURLConnection)url.openConnection();		// IOException 필요
			
			/* HTTP 응답 코드 공부하기
			 	1. 200 : 정상
			 	2. 40X : 잘못된 요청(사용자가 잘못하였다.)
			 	3. 50X : 서버측 오류(서버가 잘못하였다.)
			 */
			...
			System.out.println("User-Agent : " + userAgent);	// 만약 크롬을 이용해서 네이버에 접속하면 요청헤더는 크롬이 된다.
			
			String referer = con.getRequestProperty("referer");
			System.out.println("Referer : " + referer); 	// 이전 주소가 나온다. 보통 로그인할 때 첫 페이지가 아닌 둘러보던 이전 페이지로 돌려주는 역할.
	...
	public static void ex04() {
		...
		String apiURL = "https://www.google.co.kr/images/branding/googlelogo/1x/googlelogo_color_272x92dp.png";
		URL url = null;
		HttpURLConnection con = null;
		InputStream in = null;
		...
		try {
			...
			if(responseCode == HttpURLConnection.HTTP_OK) {
				in = con.getInputStream();
				out = new FileOutputStream("C:" + File.separator + "storage" + File.separator + "google.png");
	...
	public static void ex03_buf() {
		...
		BufferedOutputStream out = null;	// C:\storage\daum.png 로 내보내는 출력 스트림
	...
	public static void ex05() {
		...
		File file = new File("C:" + File.separator + "storage", "다운로드.txt");
		...
		try {
			...
			if(responseCode == HttpURLConnection.HTTP_OK) {
				...
				reader = new InputStreamReader(con.getErrorStream());	// 실패시 에러 스트림으로 만든다.
			}
	...
	public static void ex06() {
		/*
		 	인코딩(암호화) : 업무 데이터를 UTF-8 방식으로 암호화
	...
	public static void ex07_practice() {
		
		// 1시간마다 갱신되는 전국 날씨 정보
		String apiURL = "http://www.kma.go.kr/XML/weather/sfc_web_map.xml";
		
		// storage/sfc_web_map.xml로 다운받기
		URL url = null;
		HttpURLConnection con = null;
		File file = null;
		BufferedReader reader = null;
