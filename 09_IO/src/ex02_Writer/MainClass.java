...
public class MainClass {
	...
	public static void ex01() {
		...
		// 출력 스트림 생성
		FileWriter fw = null;	
		try {							// 일반 try문이므로 클로즈가 필요하다 실행은 되지만 후에 문제가 생길 가능성이 있다.
			...
			fw.write(str);				// 반드시 예외 처리가 필요한 코드
			
			System.out.println("ex01.txt 파일이 생성되었다.");
		} catch(IOException e) {
			e.printStackTrace(); 		// 개발 국룰 코드 트레이스로 추적
		} finally {						// 예외 발생 여부와 상관없이 반드시 실행
			try {						// fw 초기화를 null(보통 null로 초기화함)로 하였으니 null이 아닐때 클로즈
				if(fw != null) {
	...
	public static void ex02() {
		...
		try (FileWriter fw = new FileWriter(file)) {		// try에 스트림 생성을 넣어주면 클로즈 생략이 가능
			char[] cbuf = {'a', 'b', 'c', 'd', 'e'};
			String str = "abcde";
	...
	public static void ex03() {
		...
		try {
			...
			try {
				// 보조 스트림을 사용한 경우 보조 스트림을 닫으면 메인 스트림도 함께 닫힌다.
				if(bw != null) {
					bw.close();
				}
			} catch(IOException e) {
	...
	public static void ex05() {
		...
		/*
		 	...
		 	2. 줄 바꿈이 자주 필요한 경우에 println() 메소드 사용을 위해서 사용.
		 */
		...
		try {
			...
			e.printStackTrace();
				...
				out.close();	// close 메소드 내부에서 IOException을 처리 하고있다.
		}
