...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		myCalc.start();		// start 메소드는 run 메소드를 스스로 호출. main은 myCalc 스레드의 동작 시작만 처리하고 동작 완료까지 기다리지 않는다.
		
		Calculator yourCalc = new Calculator("상철");	// yourCalc 스레드 생성
		yourCalc.start();	// start 메소드는 run 메소드를 스스로 호출. main은 yourCalc 스레드의 동작 시작만 처리하고 동작 완료까지 기다리지 않는다.
