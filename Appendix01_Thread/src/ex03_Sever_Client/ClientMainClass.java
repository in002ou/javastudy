...
public class ClientMainClass {
	...
	public static void main(String[] args) {
		...
		try {
			...
			socket = new Socket();
			socket.connect(new InetSocketAddress("localhost", 9090));	// 서버 주소는 localhost, 포트번호는 9090이다.
			...
			writer = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));	// socket 은 바이트 타입이므로 OutputStreamWriter로 읽어서 보낸다
